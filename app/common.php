 ...
 * @param   $content      内容
 */
function action_log($type,$data_id,$action_name,$content){
    $type = Record::getTypeValue($type);
    $data = compact("type", "data_id", "action_name","content");
    $data['create_time'] = time();
    $data['user_id'] = request()->uid();
    return Db::name('action_log')->insert($data);
}
