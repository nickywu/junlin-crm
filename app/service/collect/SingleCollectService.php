...
use app\model\collect\SingleCollect;
use app\model\contract\ContractProduct;
use app\model\product\Product;
use core\exception\FailedException;
use core\facade\Util;

class SingleCollectService extends BaseService
{
    public function __construct(SingleCollect $model)
    {
        $this->model = $model;
    }
    ...
    public function save(array $data)
    {
        ...
        if ($result) {
            action_log('collect_single', $result, '新增', '新增单次服务收款');
        }
    ...
    public function update(int $id, array $data): bool
    {
        ...
        if ($result) {
            $logContent = '更新单次服务收款，变更字段：';
            $logContent .= Util::formatChangedFields($changed);
            action_log('collect_single', $id, '更新', $logContent);
        }
        return true;
    }
    ...
    public function delete($collect_id)
    {
        ...
        foreach ($collect_id as $v) {
            ...
            if ($result) {
                action_log('collect_single', $v, '删除', '删除单次服务收款记录');
            }
        }
        if (empty($errorInfo)) {
            return true;
