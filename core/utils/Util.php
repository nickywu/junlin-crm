...
class Util
{
     ...
     * @param array $ignoreFields 忽略字段（可选）
     * @return array ['field' => ['old' => '', 'new' => '']]
     */
    public  function getChangedFields(array $oldData, array $newData, array $ignoreFields = []): array
    {
        $changed = [];
    ...
    /**
     * 格式化变更字段为日志描述字符串（字段别名为可选）
     *
     * @param array $changedFields 变更字段数组
     * @param array $fieldLabels 字段别名映射 ['name' => '客户名称']，可不传
     * @param array $timeFields 需要格式化的时间字段 ['create_time', 'update_time']
     * @param string $glue 分隔符，默认是 "；"
     * @return string
     */
    public static function formatChangedFields(
        array $changedFields,
        array $fieldLabels = [],
        array $timeFields = [],
        string $glue = "；"
        ...
        foreach ($changedFields as $key => $item) {
            // 如果传了字段别名，并且当前字段存在，则使用别名，否则保留原字段名
            ...
            if (in_array($key, $timeFields)) {
                $old = is_numeric($old) ? date('Y-m-d H:i:s', $old) : $old;
                $new = is_numeric($new) ? date('Y-m-d H:i:s', $new) : $new;
            }

            $desc[] = "{$label}『{$old} => {$new}』";
        }

        return implode($glue, $desc);
    }
