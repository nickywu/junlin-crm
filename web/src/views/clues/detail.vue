<template>
  ...
  >
    <template #extra>
      <s-button icon="edit-outlined" type="primary" @click="emit('edit')">
        编辑
      </s-button>
      <s-button icon="retweet-outlined" @click="onTransform"> 转换 </s-button>
    ...
    <div class="view-wrap">
      <a-tabs :animated="false" v-model:activeKey="state.tabKey" class="pl-[10px]">
        ...
        <a-tab-pane key="cluesDetails" tab="详情信息" />
...
import { read, transform, destroy } from "@/api/clues";
...
const onTransform = () => {
  createConfirm({
    title: "提示",
    content: `确定要将『${state.data.name}』转换为客户吗?`,
    centered: true,
    onOk: () => {
      return transform(props.id).then(res => {
        responseNotice(res);
        refresh(true);
        closeDrawer();
      });
